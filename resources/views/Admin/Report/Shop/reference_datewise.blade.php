@extends('Master.main')
@section('title')
    Shop Payment List Reference Wise
@endsection
@section('content')
                ...
                <div class="sparkline13-list">
                    <div class="sparkline13-hd">
                        <div class="main-sparkline13-hd">
                            <h1>Shop Payment List <small>( Reference Wise )</span></h1>
                        </div>
                    </div>
                    ...
                    <div class="sparkline13-graph">
                        <div class="datatable-dashv1-list custom-datatable-overright">
                            <div id="toolbar" class="form-inline">
                                <form action="{{ route('shop.payment.reference.wise') }}" method="get"> 
                                    @csrf
                                    ...
                                    <select class="form-control" name="mobile" required>
                                        ...
                                        <option value="{{ $reference->mobile }}">{{ $reference->name }}</option>
                                        @endforeach
                                    </select>
                                    <label>&nbsp; From : </label>
                            ...
                            <table id="table" data-toggle="table" data-pagination="true" 
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th data-field="commission" data-editable="false">Commission (Tk.)</th>
                                    </tr>
                                </thead>
                                <tbody>
...
<script>
    $('.Print').click(function () {
        $.ajax({
            url: "{{ route('shop.payment.reference.wise.print') }}",
            type: 'GET',
            success: function (){
                window.open('{{ route('shop.payment.reference.wise.print') }}','_blank');
            }
        });
    });
</script>
